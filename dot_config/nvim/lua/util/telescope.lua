...
local open_win = function(enter, opts)
  ...
  if (config.title or config.footer) and (config.border == nil or config.border == "none") then
    config.border = "solid"
  end
...
local update_win = function(window, opts)
  if window and window.winid and vim.api.nvim_win_is_valid(window.winid) then
    vim.api.nvim_win_set_config(
      window.winid,
      vim.tbl_deep_extend("force", vim.api.nvim_win_get_config(window.winid), opts)
    )
  end
end
...
function M.bottom_pane(picker)
  local function get_configs()
    ...
    local preview_ratio = fn.map_range(80, 150, 3, 2, vim.o.columns)

    local preview_width = math.floor(vim.o.columns / preview_ratio) - 2
    local results_width = hide_preview and vim.o.columns or (math.floor(vim.o.columns - preview_width) - 3)
...
function M.flexible(picker)
  local function get_configs()
    ...
    local fn = require("util.fn")
    local preview_ratio = fn.map_range(100, 150, 3, 2.2, vim.o.columns)
    ...
    if vim.o.columns > 120 then
      ...
      local results_height = (hide_preview and height or math.floor(height / 2)) - 2
      res.results = {
        width = width,
