...
function List:resolve_displayed(displayed, length)
  ...
  for _, removal in ipairs(to_remove) do
    Extensions.extensions:emit(Extensions.event_names.REMOVE, { list = self, item = removal.item, idx = removal.idx })
  end
  ...
  for i = 1, length do
    ...
    if display_item == "" then
      ...
      if current_index == -1 then
        ...
        Extensions.extensions:emit(Extensions.event_names.ADD, { list = self, item = new_list[i], idx = i })
      else
        -- Existing item
...
local function prepopulate(list)
  ---@diagnostic disable-next-line: undefined-field
  if list.config.prepopulate and (list:length() == 0 or list.config.automated) then
